...
const checkMember = node({
  ...
  config: {
    ...
    parameters: {
      method: 'POST',
      url: 'https://api.telegram.org/bot@@cfg:TELEGRAM_BOT_TOKEN@@/getChatMember',
      sendBody: true,
      contentType: 'json',
      specifyBody: 'json',
      jsonBody: expr('{{ JSON.stringify({ chat_id: "@@cfg:REQUIRED_CHANNEL@@", user_id: Number($json.userId) }) }}'),
