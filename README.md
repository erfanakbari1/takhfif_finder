...
| **Takhfif Finder — Telegram Bot** | وب‌هوک ربات. درخواست بدون هدر `X-Telegram-Bot-Api-Secret-Token` درست اصلاً اجرا نمی‌شود. |
...
> takhfifhot.com و myclub.snapp.ir/vendors و takhfifan.com و offaro.com از طریق کانال‌های تلگرامی/بخش‌های عمومی پوشش داده می‌شوند. کدهای این سایت‌ها پشت لاگین یا سشن است.
...
- **ساخت ورک‌فلوها برای سرور خودتان**:
  1. `config.example.json` را به `config.json` کپی کنید و آدرس n8n و شناسه‌ی جدول‌ها و ورک‌فلوی Engine را در آن بنویسید. این فایل در git نادیده گرفته می‌شود.
